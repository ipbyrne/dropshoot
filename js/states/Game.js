...
DropShoot.GameState = {
    ...
    // REGULAR ENEMIES
    createEnemy:function(x, y, health, key, scale, range, speed, time){
        var enemy = this.enemies.getFirstExists(false);
        
        if(!enemy) {
            enemy = new DropShoot.Enemy(this.game, x, y, key, health);
            enemy.anchor.setTo(0.5);
            enemy.sx = enemy.x;
            this.enemies.add(enemy);
        }
        // Reset the enemy.
        enemy.reset(x, y, health, key, scale, range, speed, time);
    },
    scheduleNextEnemy: function() {
        ...
        this.nextEnemyTimer = this.game.time.events.add(nextTime, function() {
            ...
            this.createEnemy(nextEnemy.x, nextEnemy.y, nextEnemy.health, nextEnemy.key, nextEnemy.scale, nextEnemy.range, nextEnemy.speed, nextEnemy.time);
    ...
    // FLYING ENEMIES
    createFlyingEnemy:function(x, y, health, key, scale, range, speedX, speedY, time){
        var flyingEnemy = this.flyingEnemies.getFirstExists(false);
        
        if(!flyingEnemy) {
            flyingEnemy = new DropShoot.FlyingEnemy(this.game, x, y, key, health);
            flyingEnemy.anchor.setTo(0.5);
            this.flyingEnemies.add(flyingEnemy);
        }
        // Reset the enemy.
        flyingEnemy.reset(x, y, health, key, scale, range, speedX, speedY, time);
    },
    scheduleNextFlyingEnemy: function() {
        ...
        this.nextFlyingEnemyTimer = this.game.time.events.add(nextFlyingTime, function() {
            // Run through each of the enemyData elements inside the Json File
            this.createFlyingEnemy(nextFlyingEnemy.x, nextFlyingEnemy.y, nextFlyingEnemy.health, nextFlyingEnemy.key, nextFlyingEnemy.scale, nextFlyingEnemy.range, nextFlyingEnemy.speedX, nextFlyingEnemy.speedY, nextFlyingEnemy.time);
